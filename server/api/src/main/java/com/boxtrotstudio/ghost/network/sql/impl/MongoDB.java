...
import com.boxtrotstudio.ghost.utils.PasswordHash;
...
public class MongoDB implements SQL {
    ...
    public void updatePlayerData(PlayerUpdate data) {
        Document query = new Document().append(Constants.ID, data.getId());
    ...
    public PlayerData fetchPlayerData(String username, String password) {
        Document doc = playerCollection.find(new Document().append(Constants.USERNAME, username)).first();
        if (doc == null)
            return null;

        String hash = doc.get(Constants.HASH, String.class);
        if (hash == null)
            return null;
    ...
    public boolean usernameExists(String username) {
        return playerCollection.find(new Document().append(Constants.USERNAME, username)).first() != null;
    }

    @Override
    public boolean displayNameExist(String displayName) {
        return playerCollection.find(new Document().append(Constants.DISPLAY_NAME, displayName)).first() != null;
    }
