...
namespace Ghost.Core.Sharp2D_API
{
    public class Animation : ICloneable
    {
        ...
        public bool Playing
        {
            ...
            set
            {
                if (Owner == null) //This is a empty animation
                {
                    ...
                    {
                        ISpriteWorld world = ParentAnimation.Owner.CurrentWorld;
                        world.RemoveSprite(ParentAnimation.ModuleOwner.ChildAnimationPlaying.Owner);
                    }
        ...
        public static void Combind(Animation inherit, ref Animation @override, bool includeAnimations = true)
        {
            ...
            if (string.IsNullOrWhiteSpace(@override.origin_type))
            {
                @override.origin_type = inherit.origin_type;
            }
            if (string.IsNullOrWhiteSpace(@override.parent_origin_type))
            {
                @override.parent_origin_type = inherit.parent_origin_type;
            }
            if (string.IsNullOrWhiteSpace(@override.sprite))
            {
            ...
            if (string.IsNullOrWhiteSpace(@override.inherit_from))
            {
