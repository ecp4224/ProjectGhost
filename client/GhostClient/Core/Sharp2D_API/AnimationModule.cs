...
namespace Ghost.Core.Sharp2D_API
{
    public class AnimationModule : IModule
    {
        ...
        public void OnUpdate()
        {
            ...
            if (_lastTick + CurrentlyPlayingAnimation.Speed <= tickCount)
            {
                _lastTick = tickCount;
    ...
    public sealed class BasicAnimatedSprite : ModuleSprite
    {
        public BasicAnimatedSprite(string path)
        {
            Texture = GhostClient.Ghost.CurrentGhostGame.Content.Load<Texture2D>(path);
            Width = Texture.Width;
